...
本阶段在 Day22 五舵机动作组控制基础上，增加了五指角度校准和动作安全保护。
...
本阶段在 Day27 完整链路联调的基础上，对 Linux Gateway 进行通信稳定性优化。

优化内容：

1. 日志增加时间戳，方便定位问题。
2. MQTT 客户端增加自动重连配置。
3. JSON 解析函数增强，支持 `"cmd":"OPEN"` 和 `"cmd": "OPEN"` 两种格式。
4. 增加命令去重机制，避免同一手势短时间内重复刷屏。
5. STOP 急停命令不参与去重，保证安全优先级最高。
6. UART 串口发送增加失败重试机制。
7. 无效命令和缺失 `cmd` 字段的消息不会下发 STM32。

优化后，Linux Gateway 的职责更加清晰：
